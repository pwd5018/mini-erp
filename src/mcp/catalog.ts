...
export const readToolDefinitions: AgentToolDefinition[] = [
  {
    ...
    parameters: { type: "object", properties: { startDate: { anyOf: [{ type: "string", format: "date" }, { type: "null" }] }, endDate: { anyOf: [{ type: "string", format: "date" }, { type: "null" }] }, customerId: { anyOf: [{ type: "string" }, { type: "null" }] }, priority: { anyOf: [{ type: "string", enum: ["HIGH", "NORMAL", "LOW"] }, { type: "null" }] } }, required: ["startDate", "endDate", "customerId", "priority"], additionalProperties: false },
  },
  ...
  { name: "get_replenishment_requests", description: "Read existing replenishment requests, optionally filtered by product or status.", parameters: { type: "object", properties: { productId: { anyOf: [{ type: "string" }, { type: "null" }] }, status: { anyOf: [{ type: "string", enum: ["PENDING", "APPROVED", "COMPLETED", "CANCELLED"] }, { type: "null" }] } }, required: ["productId", "status"], additionalProperties: false } },
];
