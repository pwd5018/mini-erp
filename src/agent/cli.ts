...
console.log(JSON.stringify({ sessionId: run.sessionId, response: run.response, findings: run.findings, dataGaps: run.dataGaps, toolCalls: run.toolCalls, rounds: run.rounds }, null, 2));
} finally {
  await client.close();
  db.close();
}
